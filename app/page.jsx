...
function getInterviewDateTime(candidate) {
...
function scheduleBefore(candidate, minutesBefore) {
...
function renderTemplate(templateText, candidate, settings) {
    ...
    return templateText.replace(/\{(\w+)\}/g, (_, key) => values[key] ?? `{${key}}`);
}
...
function EmptyState({ title, text }) {
    return (<div className="rounded-3xl border border-dashed border-slate-200 bg-white/60 p-8 text-center">
...
function Section({ title, desc, action, children }) {
    return (<section className="glass rounded-[2rem] p-5 lg:p-6">
...
function AppSidebar({ active, setActive }) {
    return (<aside className="glass sticky top-4 h-fit rounded-[2rem] p-4 lg:w-72">
      ...
      <nav className="grid gap-2">
        {tabs.map((tab) => (<button key={tab.key} type="button" onClick={() => setActive(tab.key)} className={cx('flex items-center justify-between rounded-2xl px-4 py-3 text-sm font-black transition', active === tab.key ? 'bg-teal-700 text-white shadow-lg shadow-teal-700/20' : 'bg-white/60 text-slate-600 hover:bg-white')}>
...
function StatCard({ icon, label, value, hint }) {
    return (<div className="glass rounded-[1.7rem] p-5">
...
function DashboardView({ candidates, logs, setActive }) {
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <Section title="مصاحبه‌های نزدیک" desc="کاندیداهایی که باید برایشان یادآوری یا پیام هماهنگی ارسال شود.">
          <div className="space-y-3">
            {upcoming.map((c) => (<div key={c.id} className="flex flex-col gap-3 rounded-3xl border border-slate-100 bg-white/70 p-4 md:flex-row md:items-center md:justify-between">
                <div>
        ...
        <Section title="قیف جذب" desc="توزیع کاندیداها در مراحل مختلف فرایند HR.">
          <div className="space-y-3">
            ...
            return (<div key={stage} className="rounded-2xl bg-white/65 p-3">
...
function CandidateForm({ candidate, onSubmit, onCancel, settings }) {
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Field label="مرحله جذب">
          <select className="input" value={form.stage} onChange={(e) => update('stage', e.target.value)}>
            {Object.keys(stageMeta).map((stage) => <option key={stage} value={stage}>{stageMeta[stage].label}</option>)}
          </select>
        </Field>
...
function CandidatesView({ candidates, setCandidates, settings, openSms }) {
    ...
    function remove(id) {
        if (!confirm('این کاندیدا حذف شود؟'))
            return;
        setCandidates(candidates.filter((c) => c.id !== id));
    }
    return (<Section title="بانک کاندیداها" desc="ثبت اطلاعات کاندیدا، زمان مصاحبه، وضعیت جذب، امتیاز اولیه و یادداشت‌های HR." action={<div className="flex flex-wrap gap-2">
          <button className="btn btn-secondary" type="button" onClick={() => exportCsv('hr-candidates.csv', candidates)}>خروجی CSV</button>
      ...
      <div className="mb-5 grid gap-3 md:grid-cols-[1fr_220px_140px]">
        ...
        <select className="input" value={stage} onChange={(e) => setStage(e.target.value)}>
          ...
          {Object.keys(stageMeta).map((s) => <option key={s} value={s}>{stageMeta[s].label}</option>)}
        </select>
      ...
      {filtered.length ? (<div className="table-wrap">
        ...
        </div>) : <EmptyState title="کاندیدایی پیدا نشد" text="فیلترها را تغییر دهید یا کاندیدای جدید ثبت کنید."/>}
...
function InterviewsView({ candidates, setCandidates }) {
    ...
    return (<Section title="تقویم و پیگیری مصاحبه‌ها" desc="نمای عملیاتی برای HR تا بداند امروز و روزهای آینده با چه افرادی باید هماهنگ شود." action={<select className="input w-44" value={windowDays} onChange={(e) => setWindowDays(Number(e.target.value))}>
      ...
      <div className="grid gap-4">
        {list.map((c) => {
            ...
            return (<div key={c.id} className="rounded-[1.6rem] border border-slate-100 bg-white/70 p-4">
...
function SmsCenterView({ candidates, templates, settings, logs, setLogs, setCandidates }) {
    const [selectedIds, setSelectedIds] = useState(candidates.slice(0, 1).map((c) => c.id));
    ...
    function getSchedule(candidate) {
        if (mode === 'now')
            return undefined;
        if (mode === 'custom')
            return customSchedule ? new Date(customSchedule).toISOString() : undefined;
        if (mode === '24h')
            return scheduleBefore(candidate, 24 * 60);
        if (mode === '3h')
            return scheduleBefore(candidate, 3 * 60);
        if (mode === '1h')
            return scheduleBefore(candidate, 60);
        return undefined;
    }
    async function send() {
        if (!selected.length) {
        ...
        <div className="space-y-5">
          ...
          <Field label="کاندیداهای هدف">
            <div className="max-h-72 space-y-2 overflow-auto rounded-3xl border border-slate-100 bg-white/55 p-3">
              {candidates.map((c) => (<label key={c.id} className="flex cursor-pointer items-center gap-3 rounded-2xl p-3 hover:bg-white">
          ...
          {mode === 'custom' ? (<Field label="تاریخ و ساعت دلخواه">
      ...
      <Section title="پیش‌نمایش پیام" desc="متغیرها بر اساس اولین کاندیدای انتخاب‌شده جایگزین می‌شوند.">
        ...
        <div className="mt-5 grid gap-3">
          {selected.slice(0, 6).map((c) => {
            ...
            return (<div key={c.id} className="rounded-2xl bg-white/65 p-3 text-sm">
...
function TemplatesView({ templates, setTemplates }) {
    ...
    return (<Section title="مدیریت قالب‌های HR" desc="تمام سناریوهای ارتباطی منابع انسانی را به قالب تبدیل کنید تا خطای انسانی کم شود." action={<button className="btn btn-primary" type="button" onClick={openNew}>+ قالب جدید</button>}>
      <div className="grid gap-4 lg:grid-cols-2">
        {templates.map((tpl) => (<div key={tpl.id} className="rounded-[1.6rem] border border-slate-100 bg-white/70 p-5">
...
function TemplateForm({ template, onSubmit, onCancel }) {
    ...
    return (<form className="grid gap-4" onSubmit={(e) => { e.preventDefault(); onSubmit(form); }}>
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Field label="دسته‌بندی">
          <select className="input" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}>
            {['مصاحبه', 'یادآوری', 'نتیجه', 'مدارک', 'عمومی'].map((cat) => <option key={cat} value={cat}>{cat}</option>)}
          </select>
        </Field>
      </div>
...
function LogsView({ logs, setLogs }) {
    ...
    return (<Section title="گزارش ارسال پیامک" desc="هر ارسال موفق، زمان‌بندی‌شده، تستی یا ناموفق اینجا ثبت می‌شود." action={<div className="flex flex-wrap gap-2">
        <button className="btn btn-secondary" type="button" onClick={() => exportCsv('hr-sms-logs.csv', logs)}>خروجی CSV</button>
      ...
      {filtered.length ? (<div className="space-y-3">
          {filtered.map((log) => (<details key={log.id} className="rounded-[1.4rem] border border-slate-100 bg-white/70 p-4">
        ...
        </div>) : <EmptyState title="لاگی وجود ندارد" text="بعد از اولین ارسال پیامک، گزارش‌ها اینجا نمایش داده می‌شوند."/>}
    </Section>);
}
function SettingsView({ settings, setSettings }) {
    ...
    return (<Section title="تنظیمات پنل و اتصال پیامک" desc="اطلاعات عمومی HR و شماره خط پیامک را تنظیم کنید. API Key فقط در فایل .env سرور قرار می‌گیرد.">
...
function ImportExportPanel({ candidates, setCandidates }) {
    ...
    function importRows() {
        ...
        const created = lines.map((line) => {
            ...
            return {
                ...
                source: 'ورود گروهی',
                city: 'مشهد',
                stage: (interviewDate ? 'interview' : 'new'),
                interviewDate,
                interviewTime,
    ...
    return (<Section title="ورود سریع اطلاعات" desc="هر خط را با فرمت: نام، موبایل، موقعیت، تاریخ میلادی YYYY-MM-DD، ساعت HH:mm وارد کنید.">
...
export default function HRPanelPage() {
    ...
    const [candidates, setCandidatesState] = useState(() => storage.get('iho_hr_candidates', defaultCandidates));
    const [templates, setTemplatesState] = useState(() => storage.get('iho_hr_templates', defaultTemplates));
    const [logs, setLogsState] = useState(() => storage.get('iho_hr_logs', []));
    const [settings, setSettingsState] = useState(() => storage.get('iho_hr_settings', defaultSettings));
    const setCandidates = (next) => { setCandidatesState(next); storage.set('iho_hr_candidates', next); };
    const setTemplates = (next) => { setTemplatesState(next); storage.set('iho_hr_templates', next); };
    const setLogs = (next) => { setLogsState(next); storage.set('iho_hr_logs', next); };
    const setSettings = (next) => { setSettingsState(next); storage.set('iho_hr_settings', next); };
    const content = useMemo(() => {
        if (active === 'dashboard')
            return <DashboardView candidates={candidates} logs={logs} setActive={setActive}/>;
        if (active === 'candidates')
            return <div className="space-y-6"><CandidatesView candidates={candidates} setCandidates={setCandidates} settings={settings} openSms={() => setActive('sms')}/><ImportExportPanel candidates={candidates} setCandidates={setCandidates}/></div>;
        if (active === 'interviews')
            return <InterviewsView candidates={candidates} setCandidates={setCandidates}/>;
        if (active === 'sms')
            return <SmsCenterView candidates={candidates} templates={templates} settings={settings} logs={logs} setLogs={setLogs} setCandidates={setCandidates}/>;
        if (active === 'templates')
            return <TemplatesView templates={templates} setTemplates={setTemplates}/>;
        if (active === 'logs')
            return <LogsView logs={logs} setLogs={setLogs}/>;
        return <SettingsView settings={settings} setSettings={setSettings}/>;
    }, [active, candidates, templates, settings, logs]);
    return (<main className="mx-auto grid max-w-[1500px] gap-5 p-4 lg:grid-cols-[290px_1fr] lg:p-6">
